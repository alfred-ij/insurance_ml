...
```{r parse_finStatements}
#function to parse each worksheet and return a data frame
parse_finStatements <- function(ws_ns, ws_node, index){
  ...
  parsed_df <- parsed_df %>%
    ...
    add_column(col_names=c('na1','gross_liab_prem','gross_prop_prem','gross_comined_prop_liab_prem','gross_other_lines_prem',
                           ...
                           'net_nonprop_reinsurance_losses','total_net_losses','na17','na18','earned_prem','incurred_losses',
                           'incurred_loss_expenses','other_incurred_underwriting_expenses','net_underwriting_gain',
                           'na19','na20','other_underwriting_expenses_to_net_prem','na21','loss_ratio','na22','net_prem_to_policyholder_surplus','na23','na24',
...
```{r}
temp_document <- read_xml('/Users/alfredijeomah/Downloads/SelectAPage_20250616123804.xml')
temp_nameSpace <- xml_ns(temp_document)
temp_worksheets <- xml_find_all(temp_document,'.//ss:Worksheet',temp_nameSpace)
```

```{r}
parse_earnedPremium <- function(ws_ns, ws_node, index){
  ws_name <- xml_attr(ws_node, "Name", ws_ns)
  rows <- xml_find_all(ws_node, ".//ss:Row", ws_ns)
  #skip the first 6 rows
  #rows <- rows[-(1:6)]
  ...
  parsed_df <- parsed_df %>%
    ...
    mutate(lob_index=round(as.numeric(lob_index),1),
           across(c(net_written_prem,prior_unearned_prem,unearned_prem,earned_prem),as.numeric),
           year=as.integer(sub('.*December 31, (.*?) OF.*','\\1',title_str)))
  return(parsed_df)
}

parse_earnedPremium(temp_nameSpace,temp_worksheets,seq_along(temp_worksheets))
```
...
data_import<- function(input.df,input.indicators){
  ...
  for(org in c(1:nrow(input.df))){
    ...
    temp_org_df <- parse_finStatements(temp_ns,temp_wks,seq_along(temp_wks)) %>%
...
```{r,rows.print=20}
sample_df %>%
  mutate(across(c(losses,loss_adjustment_expenses,total_net_prem,unearned_prem,loss_ratio),as.numeric)) %>%
  mutate(loss_ratio_2=((losses+loss_adjustment_expenses)/loss_ratio)+unearned_prem) %>%
  select(Year,Company,loss_ratio,loss_ratio_2) %>%
  head(12)
```
