...
eda_df <- base_df %>%
  mutate(across(c(net_income,policyholders_surplus,total_assets,total_liabilities,net_cash_from_operations,
         ...
         `1yrDev_to_surplus`=`1yr_loss_development`/policyholders_surplus) %>% 
  select(Year,Company,return_on_surplus,return_on_assets,liab_to_liquidAssets,inCoursePrem_to_surplus,
         ...
         investment_yield,return_on_investments,stock_price,surplus_growth,`1yrDev_to_surplus`,
...
eda_df %>%
  na.omit() %>%
  ggplot(aes(x=Year,y=return_on_assets)) +
  geom_col() +
  theme_minimal() +
...
ref_cor <- data.frame(cor(eda_df[,names(eda_df)[sapply(eda_df,is.numeric)]]%>%na.omit()))
