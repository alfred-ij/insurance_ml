...
```{r}
temp_doc <- read_xml('/Users/alfredijeomah/Desktop/SelectAPage_20250629033730.xml')
...
parse_paidExpenses <- function(ws_ns, ws_node, index){
  ...
  parsed_df <- parsed_df %>%
    ...
    select(year,label,lae,underwriting_exp,investment_exp,total_exp)%>%
    mutate(year=year-row_number()+1)
  return(parsed_df)
}
parse_paidExpenses(temp_ns,temp_wks,seq_along(temp_wks))
```
...
eda_df <- base_df %>%
  mutate(across(c(net_income,policyholders_surplus,total_assets,total_liabilities,net_cash_from_operations,
                  ...
                  losses,loss_adjustment_expenses,net_underwriting_gain,`change_in-surplus_policyholders`,
         ...
         lossReserves_to_surplus=(losses+loss_adjustment_expenses)/policyholders_surplus,
         debt_to_equity=total_liabilities/policyholders_surplus,
         loss_ratio=(losses+loss_adjustment_expenses)/earned_prem,
  ...
  select(Year,Company,return_on_surplus,return_on_assets,liab_to_liquidAssets,inCoursePrem_to_surplus,
         ...
         GDPC1,PAYEMS,CPALTT01USM657N,MRTSSM44000USS,M2REAL) %>%
  na.omit()
```
...
eda_df %>%
  ...
  ggplot(aes(x=date,y=writtenPrem_to_surplus)) +
  geom_line() + 
  theme_minimal() +
...
#correlated combinations
  ...
  #liab_to_liquidAssets->loss_ratio
