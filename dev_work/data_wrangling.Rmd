...
parse_earnedPrem <- function(ws_ns, ws_node, index){
  ...
  parsed_df <- parsed_df %>%
    dplyr::slice(-c(1:7)) %>%
    dplyr::select(V2,V3,V8,V9,V10,V11) %>%
...
parse_paidExpenses <- function(ws_ns, ws_node, index){
  ...
  parsed_df <- parsed_df %>%
    dplyr::slice(-c(1:6)) %>%
    dplyr::select(V2,V3,V7,V8,V9,V11) %>%
    ...
    dplyr::select(year,lae,underwriting_exp,investment_exp,total_exp)%>%
    mutate(year=year-row_number()+1)
  return(parsed_df)
}
```
...
eda_df <- base_df %>%
  mutate(across(c(net_income,policyholders_surplus,total_assets,total_liabilities,net_cash_from_operations,
                  ...
                  losses,lae,net_underwriting_gain,`change_in-surplus_policyholders`,Year,
                  `1yr_loss_development`),~suppressWarnings(as.numeric(.x))),
         ...
         discrete_lossRatio=case_when(loss_ratio <= 1 ~ 1,
                                      ...
                                      .default = 3)) %>% 
  dplyr::select(Year,Company,return_on_surplus,return_on_assets,liab_to_liquidAssets,inCoursePrem_to_surplus,
...
eda_df %>%
  ...
  ggplot(aes(x=date,y=loss_ratio)) +
  geom_line() + 
  theme_minimal() +
...
```{r multi-collinearity analysis, rows.print=30}
...
correlated_cols <- c('operating_ratio','combined_ratio','liab_to_liquidAssets','debt_to_equity','M2REAL',
                     'PAYEMS','MRTSSM44000USS','return_on_assets','CPALTT01USM657N','Company','discrete_lossRatio')
...
lm_train <- eda_df[eda_df$Year<2020, names(eda_df) %notin% correlated_cols]
lm_test  <- eda_df[eda_df$Year>=2020, names(eda_df) %notin% correlated_cols] 
#separate samples into predictors (x) & target (y)
lm_XTrain <- lm_train %>% dplyr::select(-c(loss_ratio,Year)); lm_YTrain <- lm_train$loss_ratio
lm_XTest <- lm_test %>% dplyr::select(-c(loss_ratio,Year)); lm_YTest <- lm_test$loss_ratio
```
...
mlr.model <- lm(loss_ratio~.,data=lm_train%>%dplyr::select(-Year))
...
#display vif metrics
vif_df <- vif(mlr.model) %>%
  as.data.frame() %>% 
  rename(GVIF='.') 

vif_df %>%
  mutate(feature=rownames(vif_df),
...
#perform split operation...train & test samples
lasso_train <- eda_df[eda_df$Year<2020, names(eda_df) %notin% correlated_cols]
lasso_test  <- eda_df[eda_df$Year>=2020, names(eda_df) %notin% correlated_cols]
#separate samples into predictors (x) & target (y)
lasso_XTrain <- lasso_train %>% dplyr::select(-c(loss_ratio,Year)) %>% as.matrix(); lasso_YTrain <- lasso_train$loss_ratio
lasso_XTest <- lasso_test %>% dplyr::select(-c(loss_ratio,Year)) %>% as.matrix(); lasso_YTest <- lasso_test$loss_ratio
```
...
#perform split operation...train & test samples
rf_train <- eda_df[eda_df$Year<2020, names(eda_df) %notin% correlated_cols]
rf_test  <- eda_df[eda_df$Year>=2020, names(eda_df) %notin% correlated_cols]
#separate samples into predictors (x) & target (y)
rf_XTrain <- rf_train %>% dplyr::select(-c(loss_ratio,Year)); rf_YTrain <- rf_train$loss_ratio
rf_XTest <- rf_test %>% dplyr::select(-c(loss_ratio,Year)); rf_YTest <- rf_test$loss_ratio
```
...
#separate samples into predictors (x) & target (y)
xgb_XTrain <- xgb_train %>% dplyr::select(-c(loss_ratio,Year)) %>% as.matrix(); xgb_YTrain <- xgb_train$loss_ratio
xgb_XTest <- xgb_test %>% dplyr::select(-c(loss_ratio,Year)) %>% as.matrix(); xgb_YTest <- xgb_test$loss_ratio
```
...
correlated_cols <- c('operating_ratio','combined_ratio','liab_to_liquidAssets','debt_to_equity','M2REAL','Year',
                     'PAYEMS','MRTSSM44000USS','return_on_assets','CPALTT01USM657N','Company','loss_ratio','oneYRDev_to_surplus')
#define split criteria
lda_trainIndices <- createDataPartition(eda_df$discrete_lossRatio, p = 0.75, list = FALSE)
#perform split operation...train & test samples
lda_train <- eda_df[eda_df$Year<2020, names(eda_df) %notin% correlated_cols]
lda_test  <- eda_df[eda_df$Year>=2020, names(eda_df) %notin% correlated_cols]
#separate samples into predictors (x) & target (y)
