...
```{r chatgpt helper}
cat(ask_chatgpt("is zero therfore a rational answer using this method for the null model?"))
```
...
```{r null mlr modeling}
#build null model
null_mlr.model <- lm(ln(Actual_Recovery_Rate)~1,data=model_data)
...
```{r model results}
data.frame(model=c('Null MLR','MLR','Gaussian','Binomial','Binomial'),
           link=c(NA,NA,'log','probit','logit'),
           rmse=c(1.329,.633,3.019,1.938,3.533),
           r_squared=c(0,.77,.52,.52,.52))
```
